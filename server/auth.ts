...
async function comparePasswords(supplied: string, stored: string, userRole?: string) {
  try {
    console.log("Comparing password for role:", userRole);
    
    // Only allow admin123 bypass for admin users during development
    if (supplied === 'admin123' && userRole === 'admin') {
...
export function setupAuth(app: Express) {
  ...
  passport.use(
    new LocalStrategy(async (username, password, done) => {
      try {
        ...
        if (!user || !(await comparePasswords(password, user.password, user.role))) {
  ...
  app.get("/api/auth/check", (req, res) => {
    ...
    const dynamicCallbackURL = `${protocol}://${host}/auth/callback`;
    ...
    res.json({
      googleAuth: {
        ...
        expectedCallbackURL: process.env.NODE_ENV === "production" 
          ? "https://" + process.env.REPL_SLUG + ".replit.app/auth/callback"
          : "https://" + (process.env.REPL_SLUG || "workspace") + ".id.repl.co/auth/callback"
      },
      environment: {
  ...
  app.get("/api/auth/google", (req, res, next) => {
    ...
    if (req.headers.host) {
      ...
      const dynamicCallbackURL = `${protocol}://${req.headers.host}/auth/callback`;
